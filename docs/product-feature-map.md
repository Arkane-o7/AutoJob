...
| Contacts/networking CRM | Remember people and the history behind each relationship | Stores manually entered or reviewed CSV-imported contacts with links, tags, next actions, duplicate merge, and an explicit email/LinkedIn/phone/meeting/note timeline | Contact records, explicit application links, actions, and user-confirmed activity | Only explicit LinkedIn-page or email-compose navigation; CSV parsing stays local | It does not import LinkedIn connections or address books, scrape a network, read messages, infer sends from compose navigation, or send outreach |
