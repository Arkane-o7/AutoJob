...
|---|---|---|---|---|---|
| Job capture and ATS recognition | Save a job from its posting or application page | Extracts company, role, description, location, deadline, source, URL, and keywords with confidence indicators and editable fallbacks; high-confidence detections offer an in-page save/autofill prompt | Visible job-page metadata and text | A reviewed save is queued to the owner's authoritative cloud workspace; a user-specific cache supports offline work | It does not silently save every visited page or treat uncertain metadata as fact |
| Review-first autofill | Fill application fields from a chosen profile | After explicit approval in the detected-page prompt or extension popup, fills confident empty fields, supports common ATS controls and accessible frames, and can attach the locally saved resume | Active profile, answer memory, resume file, and reviewed form labels/structure | None unless the user explicitly enables localhost Ollama | It never autofills without a click, never clicks Next, Review, Apply, or Submit, and does not fill blocked sensitive, consent, CAPTCHA, or assessment fields |
...
| Answer memory | Reuse answers the user has reviewed | Matches repeated questions to profile defaults, custom answers, learned corrections, and safe fields the user completes after an explicit autofill pass; application-learned answers appear as editable/deletable custom answers and can be limited to one company domain and profile | Application-question text, user-completed answers, field fingerprints, profile ID, and optional company domain | Reviewed answers sync to the owner's private workspace and cache | It never learns passwords, identity/demographic, consent, payment, verification, or file-upload fields; it does not answer when confidence is low or reuse another profile's/company's restricted answer |
...
| Action inbox and follow-up reminders | Give the candidate one reliable daily workflow | Creates configurable follow-up actions after the user marks an application applied and combines application, contact, interview, and custom actions on Home with complete, snooze, and focused detail controls | Linked application/contact/interview, due time, priority, channel, reminder preferences, and completion state | Optional generic Chrome desktop notification after explicit permission | It does not infer submission or outreach silently, include private details in OS notifications, or contact anyone automatically |
| Contacts/networking CRM | Remember people and the history behind each relationship | Stores manually entered or reviewed CSV-imported contacts with links, tags, next actions, duplicate merge, and an explicit email/LinkedIn/phone/meeting/note timeline | Contact records, explicit application links, actions, and user-confirmed metadata-only activity | Only explicit LinkedIn-page navigation; CSV parsing stays local | It does not import LinkedIn connections or address books, scrape a network, read or draft messages, store message content, infer sends, or send outreach |
| Interview workspace | Organize interview preparation and follow-through | Stores round, format, schedule, interviewer, meeting details, research, preparation notes, question notes, and reviewed preparation/thank-you actions | Interview, application, contact, and action records | Optional calendar export or user-authorized Google Calendar sync for an action | It does not record meetings, join calls, draft or send thank-you messages, or evaluate the candidate |
| Multiple job-search profiles | Keep distinct profile/resume contexts | Supports a switchable active profile, section-based Profile editor, profile-specific answer isolation, and resume-version references | Profile index, structured profile fields, answers, and resume versions | Owner-only records sync to the authoritative workspace and user-specific cache | It does not merge profiles silently or expose one profile's restricted answers to another |
...
contacts/networking CRM, interview workspace, metadata-only relationship history,
...
- Scout does not read, draft, store, or send messages;
