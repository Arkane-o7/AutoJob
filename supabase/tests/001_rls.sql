...
select extensions.has_table('public', 'support_reports', 'private support table exists');
select extensions.is((select relrowsecurity from pg_class where oid='public.workspace_snapshots'::regclass), true, 'workspace snapshots enforce RLS');
select extensions.is((select relrowsecurity from pg_class where oid='public.applications'::regclass), true, 'applications enforce RLS');
...
select extensions.is((select count(*)::int from pg_policies where schemaname='public' and tablename='support_reports'), 0, 'normal clients receive no support-report table policy');
