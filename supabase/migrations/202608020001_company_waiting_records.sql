...
-- Companies and waiting items are private, owner-scoped CRM records. They use
-- the existing authoritative mutation path and change feed.
...
alter table public.private_records
  add constraint private_records_record_type_check check (record_type in (
    'contact_activity','company','waiting_item','reminder','answer_memory',
    'learned_answer','resume_version','knowledge_graph','settings',
    'onboarding_progress'
  )) not valid;
...
alter table public.workspace_changes
  add constraint workspace_changes_entity_type_check check (entity_type in (
    ...
    'company','waiting_item','interview','reminder','answer_memory',
    'learned_answer','resume_version','knowledge_graph','settings',
    'onboarding_progress'
  )) not valid;
...
begin
  ...
  v_updated := replace(
    ...
    '''profile'',''application'',''contact'',''contact_activity'',''company'',''waiting_item'',''interview'',''reminder'''
  );
