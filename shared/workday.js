...
(function (/** @type {any} */ root) {
    ...
    function detectWorkdayStep() {
        ...
        const headings = Array.from(document.querySelectorAll('[data-automation-id="headingText"], ' +
        ...
        for (const el of headings) {
            ...
            if (/my information/i.test(text))
                return 'My Information';
            if (/my experience/i.test(text))
                return 'My Experience';
            if (/application questions/i.test(text))
                return 'Application Questions';
            if (/voluntary disclosure/i.test(text))
                return 'Voluntary Disclosures';
            if (/self.?identify/i.test(text))
                return 'Self Identify';
            if (/^review$/i.test(text))
                return 'Review';
        }
        // Fallback: active progress-bar step tab
        const activeStep = document.querySelector('[data-automation-id*="progressBarStep"][aria-current="true"], ' +
    ...
    async function waitForAbsence(selector, timeoutMs = 4000) {
        const deadline = Date.now() + timeoutMs;
        while (Date.now() < deadline) {
     ...
     */
    async function waitForInlineFormWithLabel(labelPattern, timeoutMs = 5000) {
        const deadline = Date.now() + timeoutMs;
        while (Date.now() < deadline) {
            if (checkFormFieldLabel(labelPattern))
                return true;
            await sleep(200);
        }
        return false;
    }
    function checkFormFieldLabel(labelPattern) {
        const ffs = Array.from(document.querySelectorAll('[data-automation-id^="formField"]'));
        for (const ff of ffs) {
            const input = ff.querySelector('input:not([type="hidden"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea');
            if (!input)
                continue;
     ...
     */
    function isInlineFormOpenByLabel(labelPattern) {
    ...
    function findSkillsInput() {
        ...
        for (const ff of ffs) {
            ...
            if (/skill/i.test(label)) {
                const input = ff.querySelector('input[type="text"], input:not([type="hidden"]):not([type="submit"]):not([type="checkbox"])');
                if (input)
                    return input;
            }
        }
        // Method 2: placeholder or data-automation-id attribute fallbacks
        return (document.querySelector('[placeholder*="skill" i]') ??
     ...
     */
    function findEducationFormContainer() {
     ...
     */
    function findInlineFormContainer(idPrefix, preferLast = false) {
    ...
    function isInlineFormOpen(idPrefix) {
     ...
     */
    async function saveInlineForm(container) {
        // 1. Try the container scope first (fast path)
        if (container) {
            const inContainer = container.querySelector('[data-automation-id="saveButton"]') ??
                Array.from(container.querySelectorAll('button')).find(b => /^save$/i.test(b.textContent?.trim() ?? '') && !b.disabled);
            if (inContainer) {
                inContainer.click();
                await sleep(800);
                return true;
        ...
        const docSaveBtn = document.querySelector('[data-automation-id="saveButton"]:not([disabled])') ??
     ...
     */
    async function clickAddInSection(sectionHeadingPattern) {
        const allAddBtns = Array.from(document.querySelectorAll('button, [role="button"]')).filter(b => {
    ...
    function findNearestPrecedingHeading(el) {
        let node = el;
    ...
    function getLabelForInput(input) {
        ...
        const nativeLabels = input.labels;
     ...
     */
    async function typeForAutocomplete(input, value) {
        input.focus();
        ...
        for (const char of value) {
            ...
            if (tracker)
                tracker.setValue(current.slice(0, -1));
     ...
     */
    async function fillSpinbutton(input, value) {
    ...
    async function fillInput(input, value) {
        ...
        if (input.role === 'spinbutton' || input.getAttribute('role') === 'spinbutton') {
            await fillSpinbutton(input, value);
            return;
        }
        ...
        if (!ok && elementId) {
            await sleep(200);
            const fresh = document.getElementById(elementId);
            if (fresh) {
     ...
     */
    async function fillFieldByLabel(container, labelPattern, value) {
        if (!value)
            return false;
        // ── Strategy A: data-automation-id formField wrappers ────────────────────
        const formFields = Array.from(container.querySelectorAll('[data-automation-id="formField"]'));
        for (const ff of formFields) {
            ...
            const combo = ff.querySelector('[role="combobox"]');
            if (combo) {
                combo.focus();
                combo.click();
                await sleep(400);
                const textInput = ff.querySelector('input');
                if (textInput) {
        ...
        const inputs = Array.from(container.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea'));
    ...
    async function tickCheckboxByLabel(container, labelPattern, checked) {
        ...
        for (const cb of checkboxes) {
            const labelText = document.querySelector(`label[for="${CSS.escape(cb.id ?? '')}"]`)?.textContent?.trim() ??
            ...
            if (cb.checked !== checked) {
                ...
                if (tracker)
                    tracker.setValue(String(!checked));
                cb.click();
    ...
    function parseMonthYear(dateStr) {
        ...
        if (yr)
            return { month: '', monthNum: '', year: yr[1] };
        return null;
    }
    /**
    ...
    async function fillDateWidget(container, dateLabelPattern, month, year, monthNum) {
        ...
        for (const ff of formFields) {
            ...
            const yearEl = (ff.querySelector('input[data-automation-id="dateSectionYear-input"]') ??
        ...
        const el = container;
        const monthInput = el.querySelector(`[id*="startDate-month"], [id*="fromDate-month"], [aria-label*="Month" i]`);
        if (monthInput && (monthNum || month))
            await fillInput(monthInput, monthNum || month);
        const yearInput = el.querySelector(`[id*="startDate-year"], [id*="fromDate-year"], [aria-label*="Year" i]`);
        if (yearInput && year)
            await fillInput(yearInput, year);
    }
    // ── Work Experience inline form fill ─────────────────────────────────────────
    async function fillOpenWorkExperienceForm(entry) {
     ...
     */
    async function fillFieldByAutocompleteLabel(container, labelPattern, value) {
        if (!value?.trim())
            return false;
        const formFields = Array.from(container.querySelectorAll('[data-automation-id^="formField"]'));
        for (const ff of formFields) {
            const labelText = ff.querySelector('[data-automation-id="label"]')?.textContent?.trim() ??
                ff.querySelector('label')?.textContent?.trim() ?? '';
            if (!labelText || !labelPattern.test(labelText))
                continue;
            const combo = ff.querySelector('[role="combobox"]');
            ...
            if (!textInput) {
                wdLog(`[Workday] Autocomplete "${labelText}" — no input found`);
                continue;
            }
            ...
            const getOwnedOptions = () => {
                ...
                for (let i = listboxes.length - 1; i >= 0; i--) {
                    const opts = Array.from(listboxes[i].querySelectorAll('[role="option"]'));
                    if (opts.length > 0 && !/no items/i.test(opts[0].textContent?.trim() ?? ''))
                        return opts;
                }
                return [];
            };
    ...
    async function fillOpenEducationForm(entry) {
        ...
        if (entry.graduationYear) {
            ...
            const lastYearInput = scope.querySelector('[data-automation-id="formField-lastYearAttended"] input');
            if (lastYearInput) {
                await fillInput(lastYearInput, entry.graduationYear);
            }
            else {
                // Fallback: find year spinbuttons — the last one is "To (Actual or Expected)"
                const toYearInputs = Array.from(scope.querySelectorAll('input[role="spinbutton"][aria-label*="Year" i]'));
        ...
        const startYear = entry.startYear ?? (entry.graduationYear ? String(Number(entry.graduationYear) - 4) : null);
        if (startYear) {
            const firstYearInput = scope.querySelector('[data-automation-id="formField-firstYearAttended"] input');
            if (firstYearInput) {
                await fillInput(firstYearInput, startYear);
            }
            else {
                const yearSpinbuttons = Array.from(scope.querySelectorAll('input[role="spinbutton"][aria-label*="Year" i]'));
    ...
    async function fillDropdownByLabel(container, labelPattern, value) {
        const formFields = Array.from(container.querySelectorAll('[data-automation-id^="formField"]'));
        for (const ff of formFields) {
            const lbl = ff.querySelector('[data-automation-id="label"]')?.textContent?.trim() ??
                ff.querySelector('label')?.textContent?.trim() ?? '';
            if (!labelPattern.test(lbl))
                continue;
            const dropBtn = ff.querySelector('button[aria-haspopup="listbox"]');
            if (!dropBtn)
                continue;
            dropBtn.click();
            await sleep(300);
            ...
            let match = options.find(o => o.textContent?.trim().toLowerCase().includes(valueLower));
    ...
    // ── Skills tag-input ─────────────────────────────────────────────────────────
    async function fillSkillsTagInput(skills) {
        if (!skills.length)
            return;
        ...
        for (const skill of skills) {
            ...
            const cleanText = (o) => (o.textContent?.trim() ?? '').replace(/\s*(not\s+)?checked\s*$/i, '').toLowerCase();
    ...
    async function handleMyExperienceStep(profile) {
     ...
     */
    async function runWorkdaySpecialHandlers(profile) {
        if (!isWorkdayPage())
            return;
        ...
        if (MANUAL_STEPS.some(s => step === s)) {
            ...
            showWarning('Manual Input Needed', `Please fill out "${step}" manually, then click Save and Continue.`, 15000);
            return;
        }
        if (step === 'My Experience') {
            await handleMyExperienceStep(profile);
        }
        return { step, reviewRequired: true, manualStep: false };
    }
    ...
    ApplyOS.Workday = {
        ...
        async fillVisibleStep(profile = {}) {
            ...
            if (MANUAL_WORKDAY_STEPS.has(step)) {
                if (step !== "Review")
                    showWarning("Manual input needed", "Please complete " + step + " yourself, then review before continuing.", 15000);
