(function (/** @type {any} */ root) {
  ...
  function normalizeState(input) {
    ...
    state.reminders = state.reminders.filter((item) => {
      ...
      if (item.kind.startsWith("application_")) return Boolean(item.application_id);
      if (item.kind === "contact_follow_up") return Boolean(item.contact_id);
      if (item.kind.startsWith("interview_")) return Boolean(item.interview_id);
  ...
  function syncNextActionProjections(state) {
    ...
    state.interviews = state.interviews.map((interview) => {
      const action = firstOpenAction(state, (item) => item.interview_id === interview.id);
      return { ...interview, next_action: action?.title || "", next_action_at: action ? (action.snoozed_until || action.due_at) : null };
    });
  }
  ...
  ApplyOS.updateApplication = async function updateApplication(id, patch) {
    ...
    await ApplyOS.mutateState((state) => {
      ...
      if (["offer", "rejected", "closed"].includes(updated.status)) {
        const now = ApplyOS.nowISO();
        state.reminders = state.reminders.map((item) => item.application_id === id && item.status === "open"
  ...
  ApplyOS.deleteApplication = async function deleteApplication(id) {
    const state = await ApplyOS.mutateState((draft) => {
      ...
      const interviewIds = new Set(draft.interviews.filter((item) => item.application_id === id).map((item) => item.id));
      draft.reminders = draft.reminders.filter((item) => item.application_id !== id && !interviewIds.has(item.interview_id));
      draft.interviews = draft.interviews.filter((item) => item.application_id !== id);
      draft.contact_activities = draft.contact_activities.map((item) => item.application_id === id ? { ...item, application_id: null } : item);
  ...
  ApplyOS.markApplicationApplied = async function markApplicationApplied(id, appliedAt = ApplyOS.nowISO()) {
    ...
    await ApplyOS.mutateState((state) => {
      ...
      state.reminders = state.reminders.filter((item) => item.application_id !== id || item.source !== "system" || item.status !== "open").concat(reminders);
      updated = {
        ...application,
        status: "applied",
  ...
  ApplyOS.listActions = async function listActions(filters = {}) {
    ...
    }).sort((left, right) => new Date(left.effective_due_at).getTime() - new Date(right.effective_due_at).getTime());
  };
  ...
  ApplyOS.completeAction = async function completeAction(id) {
    ...
    const state = await ApplyOS.mutateState((draft) => {
      ...
      const hasDueReminder = draft.reminders.some((item) => item.application_id === applicationId && item.status === "open"
        && ["application_follow_up", "application_final_follow_up"].includes(item.kind)
        && new Date(item.snoozed_until || item.due_at).getTime() <= Date.now());
  ...
  ApplyOS.logContactActivity = async function logContactActivity(input = {}, options = {}) {
    ...
    await ApplyOS.mutateState((state) => {
      ...
      if (options.complete_action_id) {
        ...
        if (action) { action.status = "done"; action.completed_at = now; action.updated_at = now; }
      }
  ...
  ApplyOS.upsertInterview = async function upsertInterview(input = {}) {
    ...
    await ApplyOS.mutateState((state) => {
      ...
      if (saved.scheduled_at) {
        const due = new Date(new Date(saved.scheduled_at).getTime() - 86400000).toISOString();
  ...
  ApplyOS.deleteInterview = async function deleteInterview(id) {
    return ApplyOS.mutateState((state) => {
      ...
      state.reminders = state.reminders.filter((item) => item.interview_id !== id);
      state.contact_activities = state.contact_activities.map((item) => item.interview_id === id ? { ...item, interview_id: null } : item);
      return state;
    });
  };
